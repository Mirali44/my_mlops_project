    ...
    def initial_preprocessing(self) -> pd.DataFrame:
        ...
        if 'target' in processed_data.columns:
            try:
                ...
                processed_data['target'] = pd.to_numeric(processed_data['target'], errors='coerce').astype(float)
        ...
        if 'age' in processed_data.columns and 'gndr' in processed_data.columns:
            try:
                ...
                processed_data['age_gender_combined'] = age_filled.astype(str) + '_' + gndr_filled.astype(str)
        ...
        cols_to_drop = [col for col in processed_data.columns if col.startswith(('temp_', 'tmp_', 'test_'))]
        if cols_to_drop:
