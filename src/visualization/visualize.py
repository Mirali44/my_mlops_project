...
class MultisimVisualizer:
    ...
    def create_data_overview_dashboard(self) -> None:
        ...
        if 'age' in self.data.columns and 'target' in self.data.columns:
            try:
                ...
                data_clean = pd.DataFrame({'age': age_clean, 'target': self.data['target']}).dropna()
    ...
    def create_interactive_dashboard(self) -> None:
        ...
        try:
            ...
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=('Target Distribution', 'Age vs Tenure by Target', 
                               'Device Age Distribution', 'Feature Correlation'),
            ...
            if 'target' in self.data.columns:
                target_counts = self.data['target'].value_counts()
                fig.add_trace(
                    go.Pie(labels=['Non-Multisim', 'Multisim'], 
            ...
            if all(col in self.data.columns for col in ['age', 'tenure', 'target']):
                ...
                if len(scatter_data) > 0:
                    for target_val in [0, 1]:
                        ...
                        fig.add_trace(
                            go.Scatter(x=subset['age'], y=subset['tenure'],
                                     mode='markers',
                                     name=f'{"Multisim" if target_val == 1 else "Non-Multisim"}',
                                     marker=dict(color=self.colors['multisim'] if target_val == 1 else self.colors['non_multisim'],
                                               opacity=0.6)),
                            row=1, col=2
                        )
            ...
            if 'age_dev' in self.data.columns:
                age_dev_clean = pd.to_numeric(self.data['age_dev'], errors='coerce').dropna()
                fig.add_trace(
                    go.Histogram(x=age_dev_clean, name='Device Age',
            ...
            try:
                numerical_data = self.data.select_dtypes(include=[np.number]).iloc[:, :10]  # First 10 numeric columns
                if len(numerical_data.columns) > 1:
    ...
    def visualize_model_performance(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        axes[0,1].plot(fpr, tpr, color=self.colors['primary'], 
                       label=f'ROC curve (AUC = {roc_auc:.3f})', linewidth=2)
        ...
        axes[0,2].plot(recall, precision, color=self.colors['secondary'],
                       label=f'PR curve (AUC = {pr_auc:.3f})', linewidth=2)
        ...
        axes[1,0].hist(y_pred_proba[y_true == 0], bins=30, alpha=0.7, 
                       label='Non-Multisim', color=self.colors['non_multisim'])
        axes[1,0].hist(y_pred_proba[y_true == 1], bins=30, alpha=0.7, 
                       label='Multisim', color=self.colors['multisim'])
        ...
        axes[1,1].plot(thresholds, precision_scores, label='Precision', color=self.colors['primary'])
        ...
        if self.model and hasattr(self.model.named_steps.get('model', None), 'feature_importances_'):
    ...
    def create_interactive_model_analysis(self, y_true: np.ndarray, y_pred: np.ndarray, 
                                        y_pred_proba: np.ndarray) -> None:
        ...
        try:
            ...
            fig.add_trace(
                go.Histogram(x=y_pred_proba[y_true == 0], name='Non-Multisim',
                           marker_color=self.colors['non_multisim'], opacity=0.7),
                row=2, col=1
            )
            fig.add_trace(
                go.Histogram(x=y_pred_proba[y_true == 1], name='Multisim',
                           marker_color=self.colors['multisim'], opacity=0.7),
                row=2, col=1
            )
            ...
            if len(numerical_data_filled.columns) >= 2 and len(numerical_data_filled) > 50:
                ...
                if target is not None:
                    scatter = plt.scatter(pca_result[:, 0], pca_result[:, 1], 
                                        c=target, cmap='RdYlBu', alpha=0.6)
                    plt.colorbar(scatter, label='Target (0: Non-Multisim, 1: Multisim)')
                else:
                    plt.scatter(pca_result[:, 0], pca_result[:, 1], alpha=0.6, color=self.colors['primary'])
                ...
                if len(numerical_data_filled) <= 5000:
                    plt.subplot(1, 2, 2)
                    tsne = TSNE(n_components=2, random_state=42, perplexity=min(30, len(numerical_data_filled)-1))
                    tsne_result = tsne.fit_transform(numerical_data_filled.sample(min(1000, len(numerical_data_filled))))
                    
                    if target is not None:
                        target_sample = target.sample(min(1000, len(numerical_data_filled))).values
                        scatter = plt.scatter(tsne_result[:, 0], tsne_result[:, 1], 
                                            c=target_sample, cmap='RdYlBu', alpha=0.6)
                        plt.colorbar(scatter, label='Target (0: Non-Multisim, 1: Multisim)')
                    else:
                        plt.scatter(tsne_result[:, 0], tsne_result[:, 1], alpha=0.6, color=self.colors['secondary'])
    ...
    def create_business_insights_dashboard(self) -> None:
        ...
        try:
            ...
            if 'age' in self.data.columns and 'target' in self.data.columns:
                ...
                if len(data_age) > 0:
                    # Create age groups
                    data_age['age_group'] = pd.cut(data_age['age'], 
                                                  bins=[0, 25, 35, 45, 55, 100], 
                                                  labels=['18-25', '26-35', '36-45', '46-55', '55+'])
                    
                    adoption_by_age = data_age.groupby('age_group')['target'].agg(['count', 'sum', 'mean']).reset_index()
                    adoption_by_age['adoption_rate'] = adoption_by_age['mean'] * 100
                    
                    bars = axes[0,0].bar(adoption_by_age['age_group'], adoption_by_age['adoption_rate'],
            ...
            if available_device_cols and 'target' in self.data.columns:
                ...
                if device_adoption:
                    axes[0,1].bar(device_adoption.keys(), device_adoption.values(),
            ...
            if 'region' in self.data.columns and 'target' in self.data.columns:
                regional_data = self.data.groupby('region')['target'].agg(['count', 'sum', 'mean']).reset_index()
                regional_data['adoption_rate'] = regional_data['mean'] * 100
                
                axes[0,2].bar(regional_data['region'], regional_data['adoption_rate'],
            ...
            if 'tenure' in self.data.columns and 'target' in self.data.columns:
                ...
                data_tenure = pd.DataFrame({'tenure': tenure_clean, 'target': self.data['target']}).dropna()
                
                if len(data_tenure) > 0:
                    # Create tenure groups
                    data_tenure['tenure_group'] = pd.cut(data_tenure['tenure'],
                    ...
                    axes[1,0].plot(tenure_adoption.index, tenure_adoption.values, 
                                  marker='o', linewidth=3, markersize=8, color=self.colors['warning'])
            ...
            if 'dev_man' in self.data.columns and 'target' in self.data.columns:
                manufacturer_data = self.data.groupby('dev_man')['target'].agg(['count', 'mean']).reset_index()
                manufacturer_data = manufacturer_data[manufacturer_data['count'] >= 10]  # Filter small groups
                manufacturer_data['adoption_rate'] = manufacturer_data['mean'] * 100
                manufacturer_data = manufacturer_data.sort_values('adoption_rate', ascending=True).tail(10)
                
                axes[1,1].barh(manufacturer_data['dev_man'], manufacturer_data['adoption_rate'],
                              color=self.colors['primary'], alpha=0.7)
            ...
            if all(col in self.data.columns for col in ['tenure', 'age', 'target']):
                ...
                # Normalize values
                tenure_norm = (tenure_clean - tenure_clean.min()) / (tenure_clean.max() - tenure_clean.min() + 1e-8)
                age_norm = (age_clean - age_clean.min()) / (age_clean.max() - age_clean.min() + 1e-8)
                
                value_score = (tenure_norm + age_norm) / 2
                
                # Create value segments
                value_segments = pd.cut(value_score, bins=3, labels=['Low', 'Medium', 'High'])
                segment_data = pd.DataFrame({'segment': value_segments, 'target': self.data['target']})
                
                segment_adoption = segment_data.groupby('segment')['target'].mean() * 100
                
                axes[1,2].bar(segment_adoption.index, segment_adoption.values,
                             color=[self.colors['warning'], self.colors['primary'], self.colors['success']], alpha=0.7)
    ...
    def create_prediction_confidence_analysis(self, y_pred_proba: np.ndarray) -> None:
        ...
        try:
            ...
            confidence_scores = np.max(y_pred_proba, axis=1) if y_pred_proba.ndim > 1 else y_pred_proba
            
            # Create confidence segments
            confidence_segments = pd.cut(confidence_scores, 
                                       bins=[0, 0.6, 0.8, 0.9, 1.0],
                                       labels=['Low (0-60%)', 'Medium (60-80%)', 
                                              'High (80-90%)', 'Very High (90-100%)'])
            ...
            if y_pred_proba.ndim > 1:
                plt.hist(y_pred_proba[:, 0], bins=30, alpha=0.7, 
                        label='Non-Multisim Prob', color=self.colors['non_multisim'])
                plt.hist(y_pred_proba[:, 1], bins=30, alpha=0.7, 
                        label='Multisim Prob', color=self.colors['multisim'])
            else:
            ...
            conf_by_pred = [confidence_scores[predictions == 0], confidence_scores[predictions == 1]]
            ...
            if len(high_conf_preds) > 0:
                ...
                plt.bar(['Non-Multisim', 'Multisim'], 
                       [high_conf_counts.get(0, 0), high_conf_counts.get(1, 0)],
                       color=[self.colors['non_multisim'], self.colors['multisim']], alpha=0.7)
                ...
                for i, count in enumerate([high_conf_counts.get(0, 0), high_conf_counts.get(1, 0)]):
                    if total_high_conf > 0:
                        ...
                        plt.text(i, count + max(high_conf_counts.values) * 0.01, 
    ...
    def save_visualization_report(self, output_dir: str = 'visualization_reports') -> bool:
        ...
        try:
            ...
            print(f"✅ Visualization report saved!")
            return True
    ...
    def run_complete_visualization_suite(self, data_path: str, model_path: Optional[str] = None) -> bool:
        """
    ...
    def _preprocess_for_prediction(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'age' in X_processed.columns and 'gndr' in X_processed.columns:
            try:
                ...
                X_processed['age_gender_combined'] = age_filled.astype(str) + '_' + gndr_filled.astype(str)
            except:
                pass
        ...
        temp_cols = [col for col in X_processed.columns if col.startswith(('temp_', 'tmp_', 'test_'))]
...
def create_sample_visualizations():
    ...
    axes[0,0].pie(target_counts.values, labels=['Non-Multisim', 'Multisim'], 
...
def main():
    ...
    try:
        ...
        if visualizer.model is not None:
            ...
            X = visualizer.data.drop('target', axis=1) if 'target' in visualizer.data.columns else visualizer.data
