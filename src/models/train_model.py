...
class ModelTrainer:
    ...
    def optimize_hyperparameters(
        ...
        def objective(trial):
            ...
            params = {
                ...
                "preprocessing__pca__n_components": trial.suggest_float(
                    "pca_components", 0.85, 0.99
                ),
                "preprocessing__final_feature_selection__k": trial.suggest_int(
                    "feature_k", 15, 100
                ),
            }
    ...
    def train_complete_pipeline(
        self, data: pd.DataFrame, test_size: float = 0.2, n_trials: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        results = {
            ...
            "data_shapes": {"train": X_train.shape, "test": X_test.shape},
        }
    ...
    def save_trained_models(self, base_path: str = "models/") -> Dict[str, bool]:
        ...
        if self.best_model is not None:
            ...
            results["best"] = ModelSerializer.save_model(
                self.best_model, best_path, {**metadata, "model_type": "optimized"}
            )

            # Also save as 'latest' for easy loading
            latest_path = f"{base_path}latest_model.pkl"
            results["latest"] = ModelSerializer.save_model(
                self.best_model, latest_path, {**metadata, "model_type": "latest"}
            )
...
def train_model_from_file(
    file_path: str, test_size: float = 0.2, n_trials: int = 50
) -> Dict[str, Any]:
    """
