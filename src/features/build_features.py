...
class SmartImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        if self.columns_to_drop:
            logger.info(f"   Dropping {len(self.columns_to_drop)} columns with >{self.drop_threshold*100}% missing values")
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        for col in X.select_dtypes(include=[np.number]).columns:
            self.bounds[col] = mstats.mquantiles(
                X[col].dropna(), 
                prob=[self.limits[0], 1-self.limits[1]]
            )
...
class AdvancedFeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
                ...
                if y is not None and len(high_corr_pairs) > 0:
                    ...
                    for feat1, feat2 in high_corr_pairs:
                        try:
                            ...
                            # If correlation calculation fails, drop the second feature
                            features_to_drop.add(feat2)
        ...
        if y is not None and len(X_after_corr.columns) > 5:  # Only if we have enough features
            try:
                ...
                low_importance_features = [
                    X_after_corr.columns[i] for i, imp in enumerate(importances) 
                ...
                if features_remaining < 5:
                    ...
                    importance_df = pd.DataFrame({
                        'feature': X_after_corr.columns,
                        'importance': importances
                    }).sort_values('importance', ascending=False)
                    
                    features_to_keep = importance_df.head(5)['feature'].tolist()
                    low_importance_features = [f for f in X_after_corr.columns if f not in features_to_keep]
        ...
        if len(self.selected_features) == 0:
            ...
            logger.warning(f"   Only {len(self.selected_features)} features selected, keeping top features")
        ...
        logger.info(f"   Final selection: {len(self.selected_features)} features out of {len(X_temp.columns)}")
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(available_features) != len(self.selected_features):
            logger.warning(f"   {len(self.selected_features) - len(available_features)} selected features not found in data")
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if hasattr(X, 'columns'):
            ...
            if self.final_numerical_cols:
                ...
                num_df = pd.DataFrame(X_num_scaled, 
            ...
            if self.final_categorical_cols:
                ...
                if hasattr(X_cat_encoded, 'columns'):
                    ...
                    cat_df = pd.DataFrame(X_cat_encoded,
...
class FeatureBuilder:
    ...
    def create_preprocessing_pipeline(self) -> Pipeline:
        ...
        self.preprocessing_pipeline = Pipeline([
            ...
            ('feature_selection', AdvancedFeatureSelector(
    ...
    def transform_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"✅ Features transformed!")
        logger.info(f"   Input shape: {X.shape}")
        logger.info(f"   Output shape: {X_transformed.shape}")
        
        return X_transformed
    
    def build_features(self, data: pd.DataFrame, test_size: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
        """
...
def main():
    ...
    try:
        ...
        print("   3. Load dataset: data = processor.load_and_preprocess('multisim_dataset.parquet')")
        print("   4. Build features: X_train, X_test, y_train, y_test = feature_builder.build_features(data)")
